...
def _normalize_active_summon_entry(raw: Any) -> dict[str, Any] | None:
    ...
    normalized = {
        ...
        "status": status,
        "entityKind": _safe_lower_str(raw.get("entityKind") or "creature") or "creature",
        "isCreature": bool(raw.get("isCreature", True)),
        "actionSurfaceType": _safe_lower_str(raw.get("actionSurfaceType") or ("summoned_creature" if bool(raw.get("isCreature", True)) else "deployed_field_effect")),
        "placementRules": copy.deepcopy(raw.get("placementRules")) if isinstance(raw.get("placementRules"), dict) else {},
        "interactionModel": copy.deepcopy(raw.get("interactionModel")) if isinstance(raw.get("interactionModel"), dict) else {},
        "cleanupPolicy": copy.deepcopy(raw.get("cleanupPolicy")) if isinstance(raw.get("cleanupPolicy"), dict) else {},
