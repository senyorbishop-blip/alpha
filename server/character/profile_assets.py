...
Character profiles may contain imported portrait/token images. PDF imports can
embed those images as huge ``data:image/...;base64`` strings, which then bloat the
persisted ``char_profiles`` campaign field and every live sync. This module moves
large inline images to the existing ``/static/user_uploads`` asset directory and
keeps profile payloads bounded without touching canonical character data such as
abilities, inventory, spells, class features, HP, or conditions.
"""
...
def relocate_large_inline_images(value: Any, *, profile_label: str = "", threshold: int = DATA_IMAGE_INLINE_THRESHOLD_BYTES) -> list[dict[str, Any]]:
    """Replace large ``data:image`` strings with persisted static asset URLs.

    Mutates ``value`` in place and returns relocation metadata. Small inline data
    URLs are left untouched so tiny icons can remain embedded.
    """
...
def enforce_profile_size_caps(profile: Any, *, profile_label: str = "", string_cap: int = PROFILE_STRING_FIELD_MAX_BYTES, profile_cap: int = PROFILE_WARN_MAX_BYTES) -> list[dict[str, Any]]:
    """Truncate clearly oversized strings and warn on oversized profile docs."""
            ...
            for key in list(node.keys()):
                ...
                if isinstance(child, str):
                    ...
                    if size > string_cap:
                        ...
                        warnings.append({"path": child_path, "old_bytes": size, "new_bytes": len(node[key].encode("utf-8", errors="ignore"))})
                        logger.warning(
            ...
            for idx, child in enumerate(node):
                ...
                if isinstance(child, str):
                    ...
                    if size > string_cap:
                        ...
                        warnings.append({"path": child_path, "old_bytes": size, "new_bytes": len(node[idx].encode("utf-8", errors="ignore"))})
                        logger.warning(
                            "[char_profile_assets] truncated oversized string profile=%s path=%s old_bytes=%s cap=%s",
                            profile_label or "?",
                            child_path,
...
def sanitize_profile_persistence(profile: Any, *, profile_label: str = "") -> dict[str, Any]:
    ...
    return {"relocated": relocated, "warnings": warnings, "bytes": _json_size(profile)}
...
def _top_child_sizes(value: Any, *, limit: int = 10) -> list[tuple[str, int]]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            sizes.append((str(key), _json_size(child)))
    elif isinstance(value, list):
        for idx, child in enumerate(value):
            sizes.append((f"[{idx}]", _json_size(child)))
