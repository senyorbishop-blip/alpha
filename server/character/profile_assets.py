...
def sanitize_profile_for_websocket(profile: Any) -> Any:
    ...
    if sync_size > PROFILE_SYNC_MAX_BYTES:
        ...
        # Drop large raw/import documents.
        ...
        # Strip large computed sub-keys from charSheet (reference/rule data regenerated by client).
        if isinstance(out.get("charSheet"), dict):
            for k in _CHARSHEET_LARGE_COMPUTED_KEYS:
                child = out["charSheet"].get(k)
                if child is not None and not isinstance(child, (str, bool, int, float)):
                    sz = json_size(child)
                    if sz > _LARGE_SUBKEY_STRIP_THRESHOLD:
                        out["charSheet"][k] = {"omitted_from_sync": True, "lazy": True, "bytes": sz}
                        stripped_keys.append(f"charSheet.{k}({sz})")

        # Strip large computed sub-keys from nativeRuntime (server-side cache).
        if isinstance(out.get("nativeRuntime"), dict):
            for k in _NATIVE_RUNTIME_LARGE_KEYS:
                child = out["nativeRuntime"].get(k)
                if child is not None and not isinstance(child, (str, bool, int, float)):
                    sz = json_size(child)
                    if sz > _LARGE_SUBKEY_STRIP_THRESHOLD:
                        out["nativeRuntime"][k] = {"omitted_from_sync": True, "lazy": True, "bytes": sz}
...
# charSheet sub-keys that are computed/derived rule data — safe to omit from WS sync
# because they are large reference tables regenerated by the client or fetched separately.
_CHARSHEET_LARGE_COMPUTED_KEYS = frozenset({
    "rulesSpellbook", "spellAccess", "nativeActionCards", "nativePassives",
    "nativeClassFeatures", "nativeFeatures", "classFeatures", "spellbookEntries",
    "_rawFields",
})
# nativeRuntime sub-keys — this whole block is a server-side computed cache;
# individual large sub-keys are stripped first, then the whole block if still large.
_NATIVE_RUNTIME_LARGE_KEYS = frozenset({
    "spellAccess", "passives", "classFeatures", "bonusActions", "actions",
    "skills", "resources", "combat", "classes", "characterAudit",
})
# Minimum serialized size (bytes) for a sub-key to be stripped under the cap guard.
_LARGE_SUBKEY_STRIP_THRESHOLD = 8 * 1024
