...
def normalize_item_record(raw_item: dict | None, *, source_type: str = "inventory", source_id: str = "") -> dict:
    ...
    canonical = {
        ...
        "identity": {
            ...
            "source_revision": _safe_int(src.get("source_revision", src.get("revision", src.get("item_schema_version", 2))), 2, minimum=1, maximum=999999),
            "name": name,
            "slug": _clean_text(src.get("slug"), limit=120) or _slugify(name),
            "aliases": _ensure_list(src.get("aliases"), cap=24),
            "legacy_ids": _ensure_list(src.get("legacy_ids"), cap=16),
            "dedupe_key": _clean_text(src.get("dedupe_key"), limit=120) or re.sub(r"[^a-z0-9]", "", name.lower())[:120],
            "category": category,
        ...
        "display": {
            ...
            "description_summary": _clean_text(src.get("description_summary") or src.get("notes"), limit=300),
            "rules_summary": _clean_text(src.get("rules_summary") or src.get("effect"), limit=500),
        },
        "economy": {
            "quantity": qty,
        ...
        "equipment": {
            ...
            "versatile_damage": _clean_text(src.get("versatile_damage"), limit=16),
            "weapon_properties": _ensure_list(src.get("weapon_properties"), cap=10),
            "base_ac": _safe_int(src.get("base_ac"), 0, minimum=0, maximum=30),
            "dex_cap": _safe_int(src.get("dex_cap", -1), -1, minimum=-1, maximum=20),
        ...
        "effects": {
            ...
            "attack_bonus": _safe_int(src.get("attack_bonus"), 0, minimum=-20, maximum=30),
            "damage_bonus": _safe_int(src.get("damage_bonus"), 0, minimum=-20, maximum=30),
            "damage_dice": _clean_text(src.get("damage_dice"), limit=16),
            "damage_type": _clean_text(src.get("damage_type"), limit=32),
        },
        "usage": {
...
def to_inventory_entry(canonical: dict, *, notes: str = "", source_label: str = "", price_label: str = "") -> dict:
    ...
    entry = {
        ...
        "legacy_ids": list(identity.get("legacy_ids") or []),
        "dedupe_key": _clean_text(identity.get("dedupe_key"), limit=120),
        "source_type": identity.get("source_type") or "inventory",
        "source_id": identity.get("source_id") or "",
        "source_revision": _safe_int(identity.get("source_revision", identity.get("item_schema_version", 2)), 2, minimum=1, maximum=999999),
        ...
        "versatile_damage": equipment.get("versatile_damage") or "",
        "weapon_properties": list(equipment.get("weapon_properties") or []),
        "base_ac": _safe_int(equipment.get("base_ac"), 0, minimum=0, maximum=30),
        "dex_cap": _safe_int(equipment.get("dex_cap", -1), -1, minimum=-1, maximum=20),
        "ac_bonus": _safe_int(equipment.get("ac_bonus"), 0, minimum=-10, maximum=20),
        "strength_requirement": _safe_int(equipment.get("strength_requirement"), 0, minimum=0, maximum=30),
        "stealth_disadvantage": bool(equipment.get("stealth_disadvantage", False)),
        ...
        "description_summary": _clean_text(display.get("description_summary"), limit=300),
        "rules_summary": _clean_text(display.get("rules_summary"), limit=500),
        "attack_bonus": _safe_int(effects.get("attack_bonus"), 0, minimum=-20, maximum=30),
        "damage_bonus": _safe_int(effects.get("damage_bonus"), 0, minimum=-20, maximum=30),
        "damage_dice": _clean_text(effects.get("damage_dice"), limit=16),
        "damage_type": _clean_text(effects.get("damage_type"), limit=32),
