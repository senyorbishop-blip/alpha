(function initCharacterLevelupModal(global) {
  ...
  const CHOICE_PROFILE_OVERRIDES = {
    ...
    'circle of the land': { role: 'Terrain druid', fantasy: 'Let your magic feel anchored to place, recovery, and broad utility.', now: 'Best when you want adaptable prepared casting and zone control.', later: 'Later features reward terrain identity and spell flexibility.' },
    'circle of the moon': { role: 'Form druid', fantasy: 'Push Wild Shape into a real combat plan instead of backup utility.', now: 'Best when you want form-driven turns and frontline shape play.', later: 'Later features dramatically strengthen form identity and staying power.' },
    ...
    'berserker': { role: 'Rage striker', fantasy: 'Commit fully to pressure and ferocity.', now: 'Best when you want a loud, direct, all-in barbarian rhythm.', later: 'Later features lean into relentless offense and raw presence.' },
    ...
    'path of the world tree': { role: 'Guardian barbarian', fantasy: 'Pair primal toughness with protective reach and battlefield hold.', now: 'Best when you want a sturdier, more supportive barbarian profile.', later: 'Later features deepen your ability to hold space and protect allies.' },
  ...
  function spellCardHtml(spell, active, attrs, disabled, helper) {
    const parts = [];
  ...
  function renderSpellChoiceSection(preview, spellPlan) {
    ...
    if (cantripRequired > 0) {
      html += '<div style="margin-top:12px"><div style="font-weight:600">Pick cantrips (' + modalState.spellCantripAdds.length + ' / ' + cantripRequired + ')</div>'
        ...
        + groupedSpellCards(cantripOptions, function (spell) {
    ...
    if (levelledRequired > 0) {
      ...
      html += '<div style="margin-top:12px"><div style="font-weight:600">' + escHtml(pickLabel) + ' (' + modalState.spellLevelledAdds.length + ' / ' + levelledRequired + ')</div>'
        ...
        + groupedSpellCards(levelledOptions, function (spell) {
