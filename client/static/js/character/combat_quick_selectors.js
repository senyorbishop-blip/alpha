...
(function initCombatQuickSelectors(global) {
  ...
  function _serverWeaponRows() {
    return _safeArray(_serverQuickActions().weapon_actions).map(function (a) {
      ...
      const actionId = (typeof global._combatQuickCanonicalWeaponActionId === 'function') ? global._combatQuickCanonicalWeaponActionId(a) : String(a.action_id || a.actionId || a.id || a.name || 'weapon');
      return { id: actionId, action_id: actionId, actionId: actionId, combatCardId: actionId, itemId: _firstText(a.item_id, a.itemId, a.source_item_id, a.sourceItemId, ''), name: _firstText(a.name, 'Weapon'), source: 'weapon', sourceType: 'weapon', quickBarType: 'action', quickBarKind: isDamage ? 'use' : 'attack', quickBarLane: 'action', quickBarPickKey: 'action:' + String(actionId).toLowerCase().replace(/[^a-z0-9:_-]+/g, '-'), quickBarCanUse: !(a && a.disabled), quickBarDisabledReason: _firstText(a && a.disabled_reason, ''), quickBarAttackText: _firstText(a && a.attack_bonus, a && a.attackBonus, ''), quickBarDamageText: _firstText(a && a.damage_formula, ''), quickBarInfoSummary: isDamage ? 'Roll weapon damage after selecting the attack.' : 'Roll weapon attack.', damage_formula: _firstText(a && a.damage_formula, ''), damage_type: _firstText(a && a.damage_type, ''), attack_bonus: _firstText(a && a.attack_bonus, a && a.attackBonus, ''), attackBonus: _firstText(a && a.attack_bonus, a && a.attackBonus, '') };
    });
  }
